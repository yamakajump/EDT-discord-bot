...
module.exports = {
  ...
  async execute(member) {
    ...
    if (!welcomeChannelId) {
      console.error("La clé 'welcomeChannel' n'est pas définie dans config.json");
    } else {
      const welcomeChannel = member.guild.channels.cache.get(welcomeChannelId);
      if (!welcomeChannel) {
        console.error(`Le salon avec l'ID ${welcomeChannelId} n'a pas été trouvé`);
      } else {
    ...
    if (!pingWelcomeChannelIds || pingWelcomeChannelIds.length === 0) {
      console.error("La clé 'pingWelcomeChannelIds' n'est pas définie ou vide dans config.json");
    } else {
      pingWelcomeChannelIds.forEach(async (channelId) => {
        ...
        if (!pingChannel) {
          ...
          try {
            ...
            setTimeout(() => {
              pingMessage.delete().catch(err => console.error("Erreur lors de la suppression du ping :", err));
            }, 2000);
          } catch (err) {
            console.error("Erreur lors de l'envoi du ping dans le salon de bienvenue :", err);
          }
        }
      });
    }
